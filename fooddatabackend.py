...
SECRET_KEY = os.getenv("SECRET_KEY", "varsayilan_cok_gizli_bir_key_girin")
ALGORITHM = "HS256"
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Production için burayı frontend adresinizle sınırlayın. Örn: ["https://benim-app.com"]
    allow_credentials=True,
    allow_methods=["*"],  # GET, POST, vb. tüm metotlara izin ver
...
async def classify_and_log_meal(
    ...
    if not CLASSIFIER_URL:
        raise HTTPException(status_code=500, detail="Classifier servis adresi yapılandırılmamış.")
    ...
    resp = httpx.post(CLASSIFIER_URL, files=files)
